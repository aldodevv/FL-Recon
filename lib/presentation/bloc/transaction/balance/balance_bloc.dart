  ...
  void _onHandleBalance(
    ...
    try {
      final response = await DioApp.instance.post(
        '${AppConst.DashboardV2_1}/private/general/history/balance',
        data: {
      ...
      emit(
        state.copyWith(
          ...
          balanceHisList: balance.balanceHisList,
          lastUpdated: balance.lastUpdated,
        ),
      );
    } catch (e) {
      emit(state.copyWith(isLoadingBalance: false, messageError: '$e'));
    }
  }
  ...
  void _getGeneralBalance(
    ...
    try {
      final response = await DioApp.instance.get(
        '${AppConst.DashboardV1_2_0}/private/general/balance/${event.lang}/${event.currency}',
        options: Options(headers: {'Authorization': 'Bearer $token'}),
      );
